        ...
        async (err, result) => {
          ...
          if (!err) {
            if (result["name"] == "Cyber Security") {
              try {
                if (
                  result["duration"] != newPurchase["duration"] ||
                  result["stock"] != newPurchase["asset"]
                ) {
                  ...
                  if (result["stock"] <= newPurchase["asset"]) {
                    const asset =
                      amount + 15 * (newPurchase["asset"] - result["stock"]);
              ...
              try {
                if (result["name"] == "Back End") {
                  ...
                  result["name"] == "Front End" ||
                  result["name"] == "Full Stack"
                ) {
                  if (result["stock"] < newPurchase["asset"]) {
          ...
          const totalPrice = webDevPrice.length
            ? webDevPrice[0]
            : priceAdjustment[0] + priceAdjustment[1];
          const USD = webDevPrice.length
            ? formatter.format(webDevPrice)
